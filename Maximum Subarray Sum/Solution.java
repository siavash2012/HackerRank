...
class Result {
    ...
    public static long maximumSum(List < Long > a, long m) {
        ...
        TreeSet <Long> prefixSumSet = new TreeSet <> ();
        long max = 0;
        long prefixSum = 0;
        for (Long l: a) {
            prefixSum = (prefixSum + l) % m;
            max = Math.max(max, prefixSum);
            if (prefixSumSet.higher(prefixSum) != null) {
                max = Math.max(max, (prefixSum - prefixSumSet.higher(prefixSum) + m) % m);
            }
